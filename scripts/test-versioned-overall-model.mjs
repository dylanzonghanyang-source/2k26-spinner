...
for (const [label, model] of [["2k26", model26], ["2k27", model27]]) {
  ...
  assert.equal(model.trainingSamples, 952, `${label} must contain 952 complete cross-version samples`);
  assert.equal(model.crossValidation.mae, 1.601, `${label} combined CV MAE changed unexpectedly`);
  assert.equal(model.crossValidation.badgeSubsetMae, 1.297, `${label} combined badge CV MAE changed unexpectedly`);
...
console.log("versioned overall model OK: 952 combined samples, shared coefficients, grouped CV");
